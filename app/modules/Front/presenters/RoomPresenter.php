...
final class RoomPresenter extends BasePresenter
{
	...
	public function createComponentNewRoomForm(): Form
	{
		$form = new Form;

		$form->addText('created_by', $this->trans('general.newRoomForm.username'))
			->setRequired();

		$form->addCheckbox('burn_on_read', $this->trans('general.newRoomForm.burnOnRead'));

		$form->addPassword('password', $this->trans('general.newRoomForm.password'))
		...
		$form->addSubmit('save', $this->trans('general.newRoomForm.create'));

		$form->onSuccess[] = [$this, 'newRoomFormSucceeded'];
	...
	public function newRoomFormSucceeded(Form $form, $values)
	{
		$data = [];
		$data['password'] = Strings::trim($values->password);
		$data['created_at'] = new DateTime();
		$data['created_by'] = $values->created_by;
		$data['burn_on_read'] = $values->burn_on_read;
		...
		$row = $this->roomsRepository->findAll()->insert($data);
		$hash = HashService::idToHash($row->id, 'rooms');
		$row->update(['hash' => $hash]);

		$this->flashMessage($this->trans('general.success.roomCreated'), 'success');
		$this->redirect('Room:created', $hash);
	}
